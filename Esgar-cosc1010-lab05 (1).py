...
# Lab 05
# Lab Section: 10
# Sources, people worked with, help given to: Ashley Criger, Michael Stoll(TA)
...
for temps in max_temps: #begins for loop to go through each number in the list
    if temps > max_temp: #makes an argument to test if the number from the list is larger than the previous number
        max_temp = temps #if the number from above is larger, it will replace the previous largest number in max_temp
print(f"Max temp = {max_temp}") #prints the maximum temperature

for temps in min_temps: #begins for loop to go through each number in teh list
    if temps < min_temp: #makes an argument to test if the number from the list is smaller than the previous number
        min_temp = temps #if the number from above is smaller, it will replace the previous minimum number in min_temp
print(f"Min temp = {min_temp}") # prints the minimum temperature
...
pos_count = 0 #these all initialize the count at 0.
neg_count = 0
zero_count = 0
pos_sum = 0
neg_sum = 0
for num in numbers: #begins a loop that looks at each piece of data in the list. 
    if num > 0: #tests to see if each number is bigger than 0
        pos_count = pos_count +1 # if it is bigger than 0 it adds 1 to the count of positive numbers
        print(f"{num} is positive") #if it is positive it prints the number stating that it is positive.
    elif num < 0: # if the number isn't positive it tests to see if it is less than 0.
        neg_count = neg_count + 1 #if it is, it is negative and adds one to the count of negative.
        print(f"{num} is negative") #prints that the number is negative.
    else: #if a number doesn't come back true in either above argument
        zero_count = zero_count + 1 #it must be 0 so you add one to the zero count
        print(f"{num} is zero") #then print that it is 0
        

print(f'There are {pos_count} positive numbers') #this prints the final positive count (since it is out of the loop)
print(f'There are {neg_count} negative numbers') #this prints the final negative  count
print(f"Zero occurred {zero_count} time(s)") # this prints the final zero count
...
for num in numbers: #begins a new loop for each piece of data in numbers
    if num > 0: #checks if number is greater than 0
        pos_sum = pos_sum + num #if it is greater than zero it adds the number to the pos_sum which was initialized at 0.
    elif num < 0: #if it isn't positive it is negative, this checks that
        neg_sum = neg_sum + num #if it is negative it adds the number to the neg_sum which was initialized above at 0.
    


print(f"Sum of positive numbers {pos_sum}") #prints the final sum of pos_sum
print(f"Sum of negative numbers {neg_sum}") #prints the final sum of neg_sum
